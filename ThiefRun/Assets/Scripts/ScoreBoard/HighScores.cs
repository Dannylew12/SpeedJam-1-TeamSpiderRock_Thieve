...
{
    const string privateCode = "KayZ4qlQQk2PC3Ws8WTVEwV4mk_vrrakyIbibQ-U5TyA";  //Key to Upload New Info
    const string publicCode = "63f145178f420276809571b0";   //Key to download
    ...
    public static void UploadScore(string username, int score, float time)  //CALLED when Uploading new Score to WEBSITE
    {//STATIC to call from other scripts easily
        instance.StartCoroutine(instance.DatabaseUpload(username,score,time)); //Calls Instance
    }

    IEnumerator DatabaseUpload(string userame, int score, float time) //Called when sending new score to Website
    {
        WWW www = new WWW(webURL + privateCode + "/add/" + WWW.EscapeURL(userame) + "/" + score + "/" + Mathf.FloorToInt(time));
        yield return www;
